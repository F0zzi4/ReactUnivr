  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const messagesArray = await FirestoreInterface.getAllMessagesBySenderId(
          user.id
        );
                ...
                <div className="flex flex-col">
                  <div className="font-bold text-lg">
                    {recipients.find((rec) => rec.id === msg.recipient)?.Name +
                      " " +
                      recipients.find((rec) => rec.id === msg.recipient)
                        ?.Surname || "Unknown"}
                  </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          ...
          >
            ...
            <p className="text-sm text-gray-700 font-semibold">
              To:{" "}
              {recipients.find((rec) => rec.id === selectedMessage.recipient)
                ?.Name +
                " " +
                recipients.find((rec) => rec.id === selectedMessage.recipient)
                  ?.Surname || "Unknown"}
